...
def avg(new_data,X,Nmin,Nmax,window_size = 30):    
    ...
    b = moving_average(new_data[X],10)
...
def yes(df,X):
    grouper = np.arange(df[X].shape[0]) // 50
    k=df[X].groupby(grouper).mean()
    k=list(k)
    ma= moving_average(k,1)
    norm1 = [np.clip((float(i)-min(ma))/(max(ma)-min(ma)),0,1) for i in ma]
    return norm1
...
def avg(new_data,X,window_size = 28):    
...
while True:
    ...
    if len(txt_files)>0:
        ...
        while True:
            ...
            x=yes(df,X=' AU25_r')#Nmin=-0.5,Nmax=3.5
            if x:
                m=x[-1]
            else:
                m
